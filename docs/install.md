...
| macOS 14.3 development profile | Verified local DMG mounted; app copied to `/Applications` and launched | Downloaded Gatekeeper behavior not tested; local DMG was not quarantined | An older ad-hoc build's stale Accessibility record first caused denial; resetting only Splainit's record and granting the installed build restored TextEdit capture. Not acceptance evidence. |
